...
public class AVLTreeST<Key extends Comparable<Key>, Value> {
    ...
    public int keysLevelNumber(int level){
    	if(level==0) throw new IllegalArgumentException("O level passado é 0");
    	else{
    		return size()-level;
    	}
     ...
     */
    //metodo responsavel pela rotação dupla esquerda
    
    private Node rotateLeftDouble(Node x){
    ...
    private Node rotateRightDouble(Node x){
    ...
    private Node balance(Node x) {
        if (balanceFactor(x) < -1)          
            x = rotateLeftDouble(x);
        
        else if (balanceFactor(x) > 1)             
            x = rotateRightDouble(x);
